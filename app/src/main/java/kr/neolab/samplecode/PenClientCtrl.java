...
public class PenClientCtrl implements IPenMsgListener {
	...
	private boolean isIgnorePressure = true;
	...
	public void reqIgnorePressure(boolean ignorePressure) {
		isIgnorePressure = ignorePressure;
	}
	...
	public void onReceiveDot(int sectionId, int ownerId, int noteId, int pageId, int x, int y, int fx, int fy, int pressure, long timestamp, int type, int color) {
		// Log.e("Dots", sectionId + "," + ownerId + "," + noteId + "," + pageId + "," + x + "," + y + "," + fx + "," + fy + "," + pressure + "," + timestamp +
		// ","
		// + type + "," + color);
		sendPenDotByBroadcast(sectionId, ownerId, noteId, pageId, x, y, fx, fy, isIgnorePressure ? 255 : pressure, timestamp, type, color);
	}
	...
	private void sendPenDotByBroadcast(int sectionId, int ownerId, int noteId, int pageId, int x, int y, int fx, int fy, int pressure, long timestamp,
			int type, int color) {
		...
		//Log.e("Dots", i.getExtras().toString());
